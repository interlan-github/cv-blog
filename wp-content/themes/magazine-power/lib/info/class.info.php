...
if ( ! class_exists( 'Magazine_Power_Info' ) ) {
	...
	class Magazine_Power_Info {
		 ...
		 * Single instance.
		 *
		 * @var Magazine_Power_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Magazine_Power_Info ) ) {
				self::$instance = new Magazine_Power_Info();
		...
		public function configure() {
			...
			$this->menu_name = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'magazine-power' ), $this->theme_name );
			/* translators: 1: theme name. */
			$this->page_name = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'magazine-power' ), $this->theme_name );
			$this->tabs      = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug = $this->theme_slug . '-info';
			$this->page_url  = admin_url( 'themes.php?page=' . $this->page_slug );
			/* translators: 1: theme name. */
			$this->notice = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'magazine-power' ), esc_html( $this->theme_name ) ) . '</p>'
				.
				/* translators: 1: get started link. */
				'<p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'magazine-power' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'magazine_power_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'magazine-power' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_magazine_power_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_magazine_power_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			...
			</div><!-- .wrap .about-wrap .wt-info-wrap -->
			<?php
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%1$s() method does not exist.', 'magazine-power' ), $method );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="wt-section wt-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'magazine-power' ); ?></a>
				<?php endif; ?>
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'magazine-power-info', get_template_directory_uri() . '/lib/info/css/info' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'magazine-power-info', get_template_directory_uri() . '/lib/info/js/info' . $min . '.js', array(), '1.0.0', true );
			}
		}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'magazine_power_dismiss_status', true );
			?>
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'magazine_power_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'magazine_power_dismiss_status', 1 );
